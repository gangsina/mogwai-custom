...
public class TableEditor extends BaseEditor {
	...
	protected final static Logger logger = LoggerFactory.getLogger(TableEditor.class);
	...
	private void initialize() {
		...
		Rectangle editingViewRectangleClone =  (Rectangle)getBounds().clone();
		try {
			editingViewRectangleClone.setSize(MogwaiProperties.getInt("TABLE_EDITOR_VIEW_WIDTH"),
					MogwaiProperties.getInt("TABLE_EDITOR_VIEW_HEIGHT"));
		} catch (Exception ex) {
			logger.error(ex.getMessage(),ex);
		}

		setBounds(editingViewRectangleClone);
	}
