...
public class JGraphEditor extends DefaultScrollPane implements GenericModelEditor {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(JGraphEditor.class);
     ...
     * 确认是否包含对应的CellView
     *<br />
     *@date 2016年11月15日 下午3:22:25
     *@author <a href="bentengwu@163.com">thender</a>
     *@param aArea  
     *@return true 包含  false 不包含.
     */
    final protected boolean containCellViews(SubjectArea aArea) {
    	boolean bret	=	false;
    	CellView[] cellViews = graph.getGraphLayoutCache().getAllViews();
    	for(CellView cellView : cellViews){
    		if(cellView instanceof SubjectAreaCell){
    			if(((SubjectAreaCell) cellView).getUserObject().equals(aArea)){
    				bret = true;
    				break;
    			}
    		}
    ...
    public void commandShowSubjectArea(SubjectArea aArea) {
    	...
    	LOGGER.info("被隐藏cellView的个数.{} , All cell views {} ; "
    			,graphLayoutCache.getHiddenCellViews().length
    			,graphLayoutCache.getAllViews().length);
    	
    	Set set = graphLayoutCache.getVisibleSet();
    	LOGGER.info("显示的cell : {}",set.size());
    	
    	if(!containCellViews(aArea)){
    		SubjectAreaCell subjectAreaCell = new SubjectAreaCell(aArea);
    		graphLayoutCache.insert(subjectAreaCell);
    	}
